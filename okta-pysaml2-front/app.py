...
metadata_url_for = {
    ...
    'example-okta-com': os.environ['URL_SAML_METADATA']
...
def sp_initiated(idp_name):
    ...
    relay_state = "" if request.args.get('rd') == None else "&RelayState="+request.args.get('rd')

    redirect_url = None
    # Select the IdP URL to send the AuthN request to
    for key, value in info['headers']:
        if key is 'Location':
            redirect_url = value+relay_state
...
if __name__ == "__main__":
    ...
    if port == 5000:
        app.debug = True
    app.config.update(
        SESSION_COOKIE_DOMAIN=os.environ['COOKIE_DOMAIN']
    )
